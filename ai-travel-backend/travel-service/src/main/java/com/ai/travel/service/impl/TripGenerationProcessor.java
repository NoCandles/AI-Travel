...
public class TripGenerationProcessor {
    ...
    public void processGeneration(String tripId, CreateTripRequest request) {
        ...
        try {
            ...
            Map<String, double[]> coordinateCache = new HashMap<>();
            enrichRouteDaysWithCoordinates(routeADays, request.getDestination(), coordinateCache);
            enrichRouteDaysWithCoordinates(routeBDays, request.getDestination(), coordinateCache);
            enrichRouteDaysWithCoordinates(routeCDays, request.getDestination(), coordinateCache);
            ...
            progressEmitter.send(tripId, "progress", Map.of("stage", "packing", "message", "正在生成行李清单..."));
     ...
     */
    private void enrichRouteDaysWithCoordinates(List<JsonNode> days, String city, Map<String, double[]> cache) {
        if (days == null || days.isEmpty()) return;

        for (JsonNode day : days) {
            if (day == null || !day.has("points") || !day.get("points").isArray()) continue;
            for (JsonNode point : day.get("points")) {
                if (!(point instanceof ObjectNode)) continue;
                ObjectNode pointObj = (ObjectNode) point;

                Double existingLat = readCoordinate(pointObj, "latitude", "lat");
                Double existingLng = readCoordinate(pointObj, "longitude", "lng", "lon");
                if (isValidCoordinate(existingLat, existingLng)) {
                    pointObj.put("latitude", existingLat);
                    pointObj.put("longitude", existingLng);
                    continue;
                }

                String name = JsonUtils.getSafeText(pointObj, "name", "");
                String address = JsonUtils.getSafeText(pointObj, "address", "");
                double[] coords = resolvePointCoordinate(name, address, city, cache);
                if (coords != null) {
                    pointObj.put("latitude", coords[0]);
                    pointObj.put("longitude", coords[1]);
                }
            }
        }
    }

    private double[] resolvePointCoordinate(String name, String address, String city, Map<String, double[]> cache) {
        String keyword = StringUtils.hasText(address) ? address : name;
        if (!StringUtils.hasText(keyword)) return null;

        String cacheKey = (city == null ? "" : city) + "|" + keyword;
        if (cache.containsKey(cacheKey)) {
            return cache.get(cacheKey);
        }

        double[] coords = trySearchPoi(keyword, city);
        if (coords == null && StringUtils.hasText(name) && !name.equals(keyword)) {
            coords = trySearchPoi(name, city);
        }
        if (coords == null) {
            coords = tryGeocode(keyword, city);
        }
        if (coords == null && StringUtils.hasText(name) && !name.equals(keyword)) {
            coords = tryGeocode(name, city);
        }
    ...
    private double[] trySearchPoi(String keyword, String city) {
        try {
            Map<String, Object> result = mapService.searchPOI(keyword, city);
            Object poisObj = result == null ? null : result.get("pois");
            if (poisObj instanceof List) {
                List<?> pois = (List<?>) poisObj;
                if (!pois.isEmpty() && pois.get(0) instanceof Map) {
                    Map<?, ?> poi = (Map<?, ?>) pois.get(0);
                    Double lat = toDouble(poi.get("latitude"));
                    Double lng = toDouble(poi.get("longitude"));
            ...
            log.debug("POI coordinate resolve failed: keyword={}, city={}, error={}", keyword, city, e.getMessage());
        }
        return null;
    }

    private double[] tryGeocode(String keyword, String city) {
        try {
            Map<String, Object> result = mapService.geocode(keyword, city);
            Double lat = toDouble(result == null ? null : result.get("lat"));
            Double lng = toDouble(result == null ? null : result.get("lng"));
            if (isValidCoordinate(lat, lng)) {
                return new double[]{lat, lng};
            }
        } catch (Exception e) {
            log.debug("Geocode coordinate resolve failed: keyword={}, city={}, error={}", keyword, city, e.getMessage());
        }
        return null;
    }

    private Double readCoordinate(JsonNode node, String... fields) {
                        ...
                        if (spot.getName() != null && !spot.getName().isEmpty()) {
                            try {
                                ...
                                if (imageUrl != null && !imageUrl.isEmpty()) {
                                    ...
                                    updateTripCoverImageIfEmpty(tripId, imageUrl);
                                    count++;
                                }
                            } catch (Exception e) {
        ...
        for (int i = 0; i < daysArray.size(); i++) {
            ...
            if (points != null && points.isArray()) {
                ...
                for (int j = 0; j < points.size(); j++) {
                    ...
                    spot.setTravelGuide(JsonUtils.getSafeText(pointObj, "travelGuide", ""));
                    spot.setLatitude(readCoordinate(pointObj, "latitude", "lat"));
                    spot.setLongitude(readCoordinate(pointObj, "longitude", "lng", "lon"));
    ...
    private void updateTripCoverImageIfEmpty(String tripId, String imageUrl) {
        if (!StringUtils.hasText(tripId) || !StringUtils.hasText(imageUrl)) {
            return;
        }
        TripPlan tripPlan = tripPlanRepository.selectById(tripId);
        if (tripPlan == null || StringUtils.hasText(tripPlan.getCoverImage())) {
            return;
        }
        tripPlan.setCoverImage(imageUrl);
        tripPlanRepository.updateById(tripPlan);
    }
    ...
    /**
     * 精准替换单个景点（不动同天其它景点）。
     * 1. 定位 spot 在哪天 → 2. AI 生成同类替代（单景点）→ 3. 地理编码校验坐标真实 →
     * 4. 替换 routesJson 里那一个 spot → 5. 更新 TripSpot 表行 → 6. 返回新景点 DTO 供前端回显。
     * 坐标校验失败时换一个替代，最多重试 3 次；都失败则返回 null（前端走 needs_clarify）。
     */
    public TripSpotDTO replaceSpot(String tripId, String spotId, String hint) {
        if (tripId == null || spotId == null) return null;
        TripSpot oldSpot = tripSpotRepository.selectById(spotId);
        if (oldSpot == null) {
            log.warn("replaceSpot: 景点不存在 spotId={}", spotId);
            return null;
        }
        TripDay day = findDayBySpot(spotId, tripId);
        if (day == null) {
            log.warn("replaceSpot: 景点所属天未找到 spotId={}", spotId);
            return null;
        }
        TripPlan tripPlan = tripPlanRepository.selectById(tripId);
        if (tripPlan == null) return null;
        String destination = tripPlan.getDestination();
        String oldName = oldSpot.getName();
        String oldCategory = oldSpot.getCategory() != null ? oldSpot.getCategory() : "";

        // AI 生成替代景点（单点），带坐标校验重试
        JsonNode newPoint = null;
        for (int attempt = 0; attempt < 3 && newPoint == null; attempt++) {
            JsonNode candidate = generateReplacementSpot(destination, day, oldSpot, hint, attempt);
            if (candidate == null) continue;
            if (validateReplacementCoords(candidate, destination)) {
                newPoint = candidate;
            } else {
                log.info("replaceSpot: 替代坐标无效，重试 attempt={}, name={}",
                        attempt, JsonUtils.getSafeText(candidate, "name", ""));
            }
        }
        if (newPoint == null) {
            log.warn("replaceSpot: 3 次均未生成有效替代，放弃 tripId={}, spotId={}", tripId, spotId);
            return null;
        }

        String newName = JsonUtils.getSafeText(newPoint, "name", "");
        // 更新 TripSpot 行（保留 orderNum / arrivalTime 等编排字段，只换景点本体）
        oldSpot.setName(newName);
        oldSpot.setCategory(JsonUtils.getSafeText(newPoint, "category", oldCategory));
        oldSpot.setAddress(JsonUtils.getSafeText(newPoint, "address", ""));
        oldSpot.setLatitude(readCoordinate(newPoint, "latitude", "lat"));
        oldSpot.setLongitude(readCoordinate(newPoint, "longitude", "lng", "lon"));
        String guide = JsonUtils.getSafeText(newPoint, "travelGuide", "");
        if (StringUtils.hasText(guide)) oldSpot.setTravelGuide(guide);
        // 尝试获取新景点图片
        try {
            String imageUrl = imageService.searchImageFromPixabay(newName);
            if (StringUtils.hasText(imageUrl)) oldSpot.setImage(imageUrl);
        } catch (Exception e) {
            log.debug("replaceSpot 图片获取失败：{}", newName);
        }
        tripSpotRepository.updateById(oldSpot);

        // 同步替换 routesJson 里对应的 spot（按 name 模糊匹配当天那个点，保持三条路线一致）
        syncReplacementToRoutesJson(tripPlan, day, oldName, newPoint);

        log.info("replaceSpot 完成：tripId={}, day={}, '{}' → '{}'", tripId, day.getDay(), oldName, newName);
        return convertSpotToDTO(oldSpot);
    }

    /** AI 生成单个替代景点。attempt>0 时要求"和上次不同"以支持"换一个"。 */
    private JsonNode generateReplacementSpot(String destination, TripDay day, TripSpot oldSpot, String hint, int attempt) {
        StringBuilder prompt = new StringBuilder();
        prompt.append("目的地：").append(destination).append("。这是第").append(day.getDay()).append("天行程。");
        prompt.append("需要替换一个景点：原景点「").append(oldSpot.getName()).append("」");
        if (StringUtils.hasText(oldSpot.getCategory())) {
            prompt.append("（类型：").append(oldSpot.getCategory()).append("）");
        }
        if (StringUtils.hasText(hint)) {
            prompt.append("。用户提示：").append(hint);
        }
        prompt.append("。请推荐一个【同区域、类型相近、可替代】的真实景点，必须是真实存在、可导航到的地点。");
        if (attempt > 0) {
            prompt.append("（第").append(attempt + 1).append("次尝试，请给一个和之前不同的景点）");
        }
        prompt.append("。只输出一个景点的 JSON：{\"name\":\"景点名\",\"category\":\"类型\",\"address\":\"地址\",\"latitude\":数字,\"longitude\":数字,\"travelGuide\":\"一句话攻略\"}。不要输出数组或多个景点。");

        try {
            ShardResult shards = aiService.generateShards(destination, 1, "medium", "", "",
                    day.getDate(), day.getDate(), null, null, "drive", prompt.toString());
            JsonNode root = parseJsonSafe(shards.getShard1());
            if (root == null) return null;
            // AI 可能包在 days[0].points[0] 里，也可能直接返回单对象，两种都兼容
            if (root.has("days")) {
                JsonNode points = root.get("days").get(0).get("points");
            ...
            log.warn("generateReplacementSpot 失败：{}", e.getMessage());
            return null;
        }
    }

    /** 校验替代景点坐标真实：AI 给的坐标 + 地理编码反查，二者偏差不大才算数。 */
    private boolean validateReplacementCoords(JsonNode point, String destination) {
        Double lat = readCoordinate(point, "latitude", "lat");
        Double lng = readCoordinate(point, "longitude", "lng", "lon");
        if (!isValidCoordinate(lat, lng)) return false;
        String name = JsonUtils.getSafeText(point, "name", "");
        if (!StringUtils.hasText(name)) return false;
        try {
            Map<String, Object> geo = mapService.geocode(name, destination);
            if (geo == null) return false;
            Object latObj = geo.get("lat"), lngObj = geo.get("lng");
            if (latObj == null || lngObj == null) return false;
            double geoLat = toDouble(latObj), geoLng = toDouble(lngObj);
            if (!isValidCoordinate(geoLat, geoLng)) return false;
            // 偏差超过 50km 视为 AI 编造，判无效
            double dist = distanceKm(lat, lng, geoLat, geoLng);
            if (dist > 50) {
                log.info("replaceSpot 坐标校验：{} 地理编码偏差 {}km，判无效", name, String.format("%.1f", dist));
                return false;
            }
            // 用地理编码的坐标覆盖（更可信）
            ((com.fasterxml.jackson.databind.node.ObjectNode) point).put("latitude", geoLat);
            ((com.fasterxml.jackson.databind.node.ObjectNode) point).put("longitude", geoLng);
            return true;
        } catch (Exception e) {
            log.warn("validateReplacementCoords 地理编码失败 name={}: {}", name, e.getMessage());
            return false;
        }
    }
    ...
    private TripDay findDayBySpot(String spotId, String tripId) {
        List<TripDay> days = tripDayRepository.findByTripId(tripId);
        for (TripDay d : days) {
            List<TripSpot> spots = tripSpotRepository.findByTripDayId(d.getId());
            for (TripSpot s : spots) {
                if (s.getId().equals(spotId)) return d;
            }
        }
        return null;
    }

    /** 把替换同步到 routesJson，保持三条路线里对应那个 spot 一致 */
    private void syncReplacementToRoutesJson(TripPlan tripPlan, TripDay day, String oldName, JsonNode newPoint) {
        if (tripPlan.getRoutesJson() == null) return;
        try {
            ObjectNode routesRoot = (ObjectNode) objectMapper.readTree(tripPlan.getRoutesJson());
            JsonNode routes = routesRoot.get("routes");
            if (routes == null || !routes.isArray()) return;
            int targetDayIdx = day.getDay() - 1;
            for (JsonNode routeNode : routes) {
                ObjectNode route = (ObjectNode) routeNode;
                JsonNode days = route.get("days");
                if (days == null || !days.isArray()) continue;
                if (targetDayIdx >= days.size()) continue;
                ObjectNode dayNode = (ObjectNode) days.get(targetDayIdx);
                JsonNode points = dayNode.get("points");
                if (points == null || !points.isArray()) continue;
                for (int j = 0; j < points.size(); j++) {
                    JsonNode p = points.get(j);
                    String pname = JsonUtils.getSafeText(p, "name", "");
                    if (pname.equals(oldName) || (StringUtils.hasText(oldName) && pname.contains(oldName))) {
                        // 用新景点信息替换该点（保留 arrivalTime 等编排字段）
                        ObjectNode newP = p.isObject() ? ((ObjectNode) p).deepCopy() : objectMapper.createObjectNode();
                        newP.put("name", JsonUtils.getSafeText(newPoint, "name", ""));
                        if (newPoint.has("category")) newP.set("category", newPoint.get("category"));
                        if (newPoint.has("address")) newP.set("address", newPoint.get("address"));
                        newP.put("latitude", readCoordinate(newPoint, "latitude", "lat"));
                        newP.put("longitude", readCoordinate(newPoint, "longitude", "lng", "lon"));
                        if (newPoint.has("travelGuide")) newP.set("travelGuide", newPoint.get("travelGuide"));
                        ((ArrayNode) points).set(j, newP);
                    }
                }
            }
            tripPlan.setRoutesJson(objectMapper.writeValueAsString(routesRoot));
            ...
            log.warn("syncReplacementToRoutesJson 失败：{}", e.getMessage());
        }
    }
