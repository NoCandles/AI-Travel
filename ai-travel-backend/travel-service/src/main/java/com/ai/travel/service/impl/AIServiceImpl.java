...
public class AIServiceImpl implements AIService {
    ...
    public AIServiceImpl(RestTemplate restTemplate,
                         ...
                         @Qualifier("tripGenerationExecutor") Executor tripGenerationExecutor,
                         PromptBuilder promptBuilder) {
        this.restTemplate = restTemplate;
        this.objectMapper = objectMapper;
        this.tripGenerationExecutor = tripGenerationExecutor;
    ...
    public ShardResult generateShards(String destination, int days, String budget,
        ...
        try {
            ...
            // 根据天数动态计算max_tokens（天数越多输出越长，防止截断）
            // 每1天大约需要1500-2000 tokens的JSON输出，乘以1.5保险系数
            // DeepSeek chat 模型最大输出32000 tokens
            final int maxTokensRoute = Math.min(32000, 8000 + days * 1800);
            final int maxTokensShard4 = Math.min(24000, 6000 + days * 1200);
            log.info("动态max_tokens: routeShards={}, shard4={}, days={}", maxTokensRoute, maxTokensShard4, days);

            // 徒步专属参数（必须final，用于lambda）
            final String hikingParams = (isHiking && hikingProfile != null)
                    ? buildHikingParams(hikingProfile)
                    : "";

            // 热点数据注入（必须final，用于lambda）
            ...
            if (isHiking) {
                // 徒步模式：3条不同风格的徒步路线 + 1条装备安全指南
                CompletableFuture<String> shard1Future = CompletableFuture.supplyAsync(() ->
                        callAI(buildHikingShard1Prompt(destination, commonParams, hikingParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
                CompletableFuture<String> shard2Future = CompletableFuture.supplyAsync(() ->
                        callAI(buildHikingShard2Prompt(destination, commonParams, hikingParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
                CompletableFuture<String> shard3Future = CompletableFuture.supplyAsync(() ->
                        callAI(buildHikingShard3Prompt(destination, commonParams, hikingParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
                CompletableFuture<String> shard4Future = CompletableFuture.supplyAsync(() ->
                        callAI(buildHikingShard4Prompt(destination, commonParams, hikingParams, hotDataSection), maxTokensShard4, 0.5), tripGenerationExecutor);
            ...
            // 普通模式：原有4分片逻辑
            CompletableFuture<String> shard1Future = CompletableFuture.supplyAsync(() ->
                    callAI(promptBuilder.buildShard1Prompt(destination, days, commonParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
            CompletableFuture<String> shard2Future = CompletableFuture.supplyAsync(() ->
                    callAI(buildShard2Prompt(destination, days, commonParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
            CompletableFuture<String> shard3Future = CompletableFuture.supplyAsync(() ->
                    callAI(buildShard3Prompt(destination, days, commonParams, hotDataSection), maxTokensRoute, 0.7), tripGenerationExecutor);
            CompletableFuture<String> shard4Future = CompletableFuture.supplyAsync(() ->
                    callAI(buildShard4Prompt(destination, days, commonParams, hotDataSection), maxTokensShard4, 0.5), tripGenerationExecutor);
    ...
    private String buildCommonParams(String destination, int days, String budget,
        ...
        if (startPoint != null && !startPoint.isEmpty()) {
            ...
            // 判断是否往返：endPoint 不为空（无论是否等于 startPoint，只要都是同城往返即可）
            boolean hasReturn = endPoint != null && !endPoint.isEmpty();
            // 判断是否同城往返：起终点城市相同（如成都东站→成都南站）
            boolean sameCityReturn = hasReturn && isSameCity(startPoint, endPoint);

            if (!startPoint.equals(destination)) {
                if (hasReturn) {
                    // 往返路线：有起点也有终点，如成都东站→格聂南线→成都南站
                    p.append("⚠️【往返路线】从").append(startPoint).append("出发→游览").append(destination);
                    p.append("→").append(days).append("天后返回").append(endPoint).append("。");
                    p.append("\n📌 硬性要求：");
                    p.append("\n  1. 第一天的行程必须以「").append(startPoint).append("」作为第一个出发地点");
                    p.append("（第一天第一个景点/出发点必须是").append(startPoint).append("）！");
                    p.append("\n  2. 最后一天的行程必须以「").append(endPoint).append("」作为最后一个返回地点");
                    p.append("（最后一天最后一个景点/终点必须是").append(endPoint).append("）！");
                    if ("drive".equals(travelMode)) {
                        p.append("\n  3. 自驾路线：第一天从").append(startPoint).append("出发沿途游玩→抵达").append(destination);
                        p.append("→最后一天从").append(destination).append("自驾返回").append(endPoint);
                        if (sameCityReturn) {
                            p.append("（").append(startPoint).append("和").append(endPoint).append("同属一座城市，属于同城往返）");
                        }
                    }
                } else {
                    // 单程路线：只有起点没有终点，如成都→厦门
                    p.append("⚠️【单程路线】从").append(startPoint).append("出发→").append(destination);
                    p.append("。").append(getTravelModeText(travelMode)).append("旅行。");
                    p.append("\n📌 硬性要求：第一天的行程必须以「").append(startPoint);
                    p.append("」作为第一个出发地点（第一天第一个景点/出发点必须是").append(startPoint).append("）！");
                    if ("drive".equals(travelMode)) {
                        p.append("请规划从").append(startPoint).append("到").append(destination).append("的沿途自驾路线。");
                    } else {
                        p.append("第一天从").append(startPoint).append("出发，抵达").append(destination).append("后开始游览。");
                    }
                }
                p.append("\n");
        ...
        // 终点≠起点时的返程提示（异地返程，如成都出发→厦门→回北京）
        if (endPoint != null && !endPoint.isEmpty() && !endPoint.equals(destination)
                && !endPoint.equals(startPoint) && (startPoint == null || startPoint.isEmpty())) {
            p.append("返回终点：").append(endPoint).append("。");
            p.append("\n📌 硬性要求：最后一天的行程必须以「").append(endPoint);
            p.append("」作为最后一个返回地点（最后一天最后一个景点/终点必须是").append(endPoint).append("）！");
            p.append("最后一天从").append(destination).append("前往").append(endPoint).append("，需规划返程路线。\n");
        }
        return p.toString();
    }
    ...
    /**
     * 判断两个地点是否同属一座城市
     * 如"成都东站"和"成都南站"都属成都，返回 true
     */
    private boolean isSameCity(String a, String b) {
        if (a == null || b == null) return false;
        if (a.equals(b)) return true;
        // 提取城市名前缀（取第一个字到"站/机场/东/南/西/北/中心"之前的部分）
        String cityA = extractCityName(a);
        String cityB = extractCityName(b);
        return cityA.length() >= 2 && cityA.equals(cityB);
    }

    /**
     * 从地点名提取城市名
     * 如"成都东站"→"成都"，"双流机场"→"双流"，"天府广场"→"天府"
     */
    private String extractCityName(String location) {
        if (location == null) return "";
        // 匹配常见后缀：站、机场、广场、东站、南站、西站、北站、汽车站等
        String[] suffixes = {"火车东站", "火车南站", "火车西站", "火车北站",
                "东站", "南站", "西站", "北站", "站", "机场", "广场", "中心", "汽车站", "客运站"};
    ...
    private String buildShard2Prompt(String destination, int days, String common, String hotData) {
        return common + hotData +
                ...
                "\n\n每天安排 3-5 个景点，出发时间 8-9 点，标注到达时间(HH:MM)和游玩时长duration,小时)。" +
                "\n每个景点必须包含 travelGuide 字段：简短的游玩攻略：0-100字），包含最佳拍照位置、打卡技巧、避坑指南等实用建议。" +
                "\n同时推荐当天适合的酒店（含名称、简介、价格）。" +
                "\n\n⚠️ 必须输出正好 " + days + " 天的完整行程（第1天到第" + days + "天），一天都不能少！" +
                "\n\n请按以下 JSON 格式输出完整的" + days + " 天行程（只输出JSON，不要其他文字），" +
                "\n{\"days\":[" +
                "\n  {\"day\":1,\"date\":\"2026-06-15\",\"weather\":\"晴\",\"temperature\":\"28°C\"," +
                "\n   \"points\":[{\"name\":\"景点名\",\"address\":\"地址\",\"arrivalTime\":\"09:00\",\"duration\":2,\"category\":\"景点类型\",\"travelGuide\":\"游玩攻略：拍照技巧、打卡点、避坑指南等\"}]," +
                "\n   \"hotel\":{\"name\":\"酒店名\",\"detail\":\"简介\",\"price\":300}}," +
                "\n  ...," +
                "\n  {\"day\":" + days + ",\"date\":\"2026-06-" + (14 + days) + "\",\"weather\":\"晴\",\"temperature\":\"28°C\"," +
                "\n   \"points\":[{\"name\":\"景点名\",\"address\":\"地址\",\"arrivalTime\":\"09:00\",\"duration\":2,\"category\":\"景点类型\",\"travelGuide\":\"游玩攻略\"}]," +
    ...
    private String buildShard3Prompt(String destination, int days, String common, String hotData) {
        return common + hotData +
                ...
                "\n\n每天安排 3-5 个景点，出发时间 8-9 点，标注到达时间(HH:MM)和游玩时长duration,小时)。" +
                "\n每个景点必须包含 travelGuide 字段：简短的游玩攻略：0-100字），包含最佳游览路线、必看点、注意事项等实用建议。" +
                "\n同时推荐当天适合的酒店（含名称、简介、价格）。" +
                "\n\n⚠️ 必须输出正好 " + days + " 天的完整行程（第1天到第" + days + "天），一天都不能少！" +
                "\n\n请按以下 JSON 格式输出完整的" + days + " 天行程（只输出JSON，不要其他文字），" +
                "\n{\"days\":[" +
                "\n  {\"day\":1,\"date\":\"2026-06-15\",\"weather\":\"晴\",\"temperature\":\"28°C\"," +
                "\n   \"points\":[{\"name\":\"景点名\",\"address\":\"地址\",\"arrivalTime\":\"09:00\",\"duration\":2,\"category\":\"景点类型\",\"travelGuide\":\"游玩攻略：最佳路线、必看点、注意事项等\"}]," +
                "\n   \"hotel\":{\"name\":\"酒店名\",\"detail\":\"简介\",\"price\":300}}," +
                "\n  ...," +
                "\n  {\"day\":" + days + ",\"date\":\"2026-06-" + (14 + days) + "\",\"weather\":\"晴\",\"temperature\":\"28°C\"," +
                "\n   \"points\":[{\"name\":\"景点名\",\"address\":\"地址\",\"arrivalTime\":\"09:00\",\"duration\":2,\"category\":\"景点类型\",\"travelGuide\":\"游玩攻略\"}]," +
    ...
    private String buildHikingParams(HikingProfileDTO p) {
        ...
        if (p.getDurationType() != null) {
            ...
            switch (p.getDurationType()) {
                ...
                case "full-day": sb.append("1日（约7-10小时，）"); break;
        ...
        if (p.getDifficulty() != null) {
            ...
            sb.append("\n难度偏好：").append(p.getDifficulty()).append("是- ")
    ...
    private String buildHikingShard1Prompt(String destination, String common, String hikingParams, String hotData) {
        return "你是专业徒步路线规划师。只输出JSON，无其他内容。\n\n" +
                ...
                "\n  \"emergencyInfo\":{\"nearestVillage\":\"XX来3km\",\"emergencyPhone\":\"119\",\"evacRoute\":\"7km处可沿溪下撤到XX村\"}}," +
    ...
    private String buildHikingShard2Prompt(String destination, String common, String hikingParams, String hotData) {
        return "你是专业徒步路线规划师。只输出JSON，无其他内容。\n\n" +
                ...
                "\n将整条路线按路段拆解，每段标注距离、路况、坡度、亮点景观、休息点、风险提示。" +
                "\n\n输出格式同分片（routeName改为「探险挑战徒步路线」，路线特色不同）。";
    }
    ...
    private String buildHikingShard3Prompt(String destination, String common, String hikingParams, String hotData) {
        return "你是专业徒步路线规划师。只输出JSON，无其他内容。\n\n" +
                ...
                "\n将整条路线按路段拆解，每段标注距离、路况、坡度、亮点景观、休息点、风险提示。" +
                "\n\n输出格式同分片（routeName改为「经典环线徒步路线」，路线特色不同）。";
    }

    /**
     * 徒步分片4：装备安全/补给全套攻略（3套风格）
     */
    private String buildHikingShard4Prompt(String destination, String common, String hikingParams, String hotData) {
        return "你是专业户外领队和装备顾问。只输出JSON，无其他内容。\n\n" +
                common + hikingParams + hotData +
                "\n\n请为" + destination + "徒步提供3套装备安全/补给攻略，" +
            ...
            if (choices != null && choices.isArray() && choices.size() > 0) {
                ...
                if (jsonStart >= 0 && jsonEnd >= jsonStart) {
                    ...
                    try {
                        ...
                        log.warn("提取的JSON 无效，尝试修复：{}", e.getMessage());
                        // 尝试修复常见的JSON 错误
    ...
    private String tryFixJson(String json) {
        ...
        throw new RuntimeException("无法修复无效的JSON 响应");
    }
    ...
    public String generateSpotDescription(String spotName, String category) {
        String prompt = String.format("请为景点'%s'（类型：%s）生成一段50 字左右的介绍文字",
                spotName, category);
    ...
    public String generatePackingList(String destination, int days, String travelMode, String weatherInfo) {
        ...
        prompt.append("基于目的地天气和旅行天数为每位物品推荐合理数量，如果是自驾骑行需补充车载用品。");
        ...
        try {
            ...
            if (response != null) {
                ...
                if (choices != null && choices.isArray() && choices.size() > 0) {
                    ...
                    // 健壮提取 JSON：去掉markdown 代码块，再找第一为{ 到最后一为}
