...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginResponse loginByPhone(LoginRequest request) {
        ...
        try {
            ...
            if (existingUser != null) {
                ...
                // 新用户赠送 20 积分欢迎奖励（受积分开关控制）
                if (pointsEnabled) {
                    try {
                        pointsService.grantWelcomeBonus(user.getId());
                    } catch (Exception ex) {
                        log.warn("新用户积分发放失败：userId={}, error={}", user.getId(), ex.getMessage());
                    }
                }
    ...
    public LoginResponse guestLogin() {
        ...
        // 游客用户也赠送 20 积分欢迎奖励（受积分开关控制）
        if (pointsEnabled) {
            try {
                pointsService.grantWelcomeBonus(user.getId());
            } catch (Exception ex) {
                log.warn("游客积分发放失败：userId={}, error={}", userId, ex.getMessage());
            }
        }
